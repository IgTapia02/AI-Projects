...
public class NegaScout : PlayerBase
{
    int depth = 4;
    int nNodes = 0;
    struct Move_
    ...
    override protected GameObject SpawnPiece()
    {
        ...
        Debug.Log("Soy el Negascout");

        Move_ actualMove = new Move_(0, CloneBoard(controller.GetArrayField()), int.MinValue, 0, 1);
        PrintBoard(actualMove.Board);

        Move_ nextMove = NegaScoutSearch(actualMove, float.NegativeInfinity, float.PositiveInfinity);
    ...
    Move_ NegaScoutSearch(Move_ _actualMove, float alpha, float beta)
    {
        ...
        foreach (int move in moves)
        {
            ...
            Move_ resultMove = NegaScoutSearch(newMove, -adaptiveBeta, -Mathf.Max(alpha, bestScore));
            int currentScore = -resultMove.Value;

            if (currentScore > bestScore)
            {
                if (adaptiveBeta == beta || _actualMove.Depth >= depth - 2)
                {
                    bestScore = currentScore;
                }
                else
                {
                    resultMove = NegaScoutSearch(newMove, -beta, -currentScore);
                    bestScore = -resultMove.Value;
                }
                bestMove = resultMove;
                bestMove.Value = bestScore;
